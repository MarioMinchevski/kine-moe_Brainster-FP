...
import { PostCommentReply } from "./types";
import { motion } from "framer-motion"
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { UserType } from "../../SharedTypes/SharedTypes";
import { replyToReply } from "../../utils/replyToReply";

export function CommentReplyItem({ replyAuthorImg, replyAuthorName, replyLikes, replyText, onToggleReply, isOpen, id, postCommentId, onHandleReplyToReplyCloseTextBox, onHandleReplyToReplyToggle, onHandleReplyToReplyCommentIdSelection }: PostCommentReply) {
    const queryClient = useQueryClient()

    const [replyTextValue, setReplyTextValue] = useState(`@${replyAuthorName}`)
    ...
    const addReplyToReplyMutation = useMutation({
    ...
    const handleConfirm = async (commentToReplyTo: number) => {
        try {
            ...
            if (id)
                await addReplyToReplyMutation.mutateAsync({ id: idAsString, commentReplyText: replyTextValue, commentAuthorReplyImg, commentAuthorReplyName, postCommentId: commentToReplyTo })
            setReplyTextValue('')
            onHandleReplyToReplyToggle()
            setIsErrorMessageVisible(false)
            onToggleReply(null)
            onHandleReplyToReplyCloseTextBox()
        } catch (err) {
    ...
    const handleToggle = () => {
        if (isOpen) {
            ...
            onHandleReplyToReplyCommentIdSelection()
            onToggleReply(true);
        }
    }

    const isImageBase64 = replyAuthorImg.length > 100

    return (<>
        <div className="comment-reply">
            <div className="comments-item__posted-by-wrap">
                <div className="comment-reply-lines">
                    <div className="line-one"></div>
                    <div className="line-two"></div>
                </div>
                <div className="post-author-img-box-outer">
                    <div className="post-author-img-box">
                        {isImageBase64 ? (
                            <img src={`data:image/png;base64,${replyAuthorImg}`} alt="user-img" className="post-author-img" />
                        ) : (
                            <img src={replyAuthorImg} alt="user-img" className="post-author-img" />
                        )}
                    </div>
                </div>
                <div className="comments-item__posted-by-wrap__inner-wrap">
                    <span className="comment-by-user">{replyAuthorName}</span>
                    <p className="comments-title">{replyText}</p>
                </div>
            </div>
            <div className="comments-item__reactions-wrap">
                ...
                <div className="reactions-wrap__inner-alt">
                    ...
                    <span className="likes-num">{replyLikes}</span>
                </div>
            </div>
        </div>
        {isOpen &&
            ...
            >
                ...
                ></textarea>
                <button
                    onClick={() => handleConfirm(postCommentId)}
                    className="submit-reply-btn">Reply</button>
